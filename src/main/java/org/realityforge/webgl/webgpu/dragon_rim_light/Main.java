package org.realityforge.webgl.webgpu.dragon_rim_light;
  ...
  implements EntryPoint
{
  ...
  private void onStart( @Nonnull final GPUDevice device )
  {
    ...
    final String vertexShader =
      ...
      "  var fragmentToCamera = normalize(uniforms.cameraPosition - input.worldPosition);\n" +
      "  var rimAmount = pow(1.0 - max(0.0, dot(normal, fragmentToCamera)), 2.0);\n" +
      "  var rimLightColor = vec3<f32>(1.0, 1.0, 1.0);" +
      "  var output : FragmentOutput;\n" +
      "  output.color = vec4<f32>(rimLightColor * rimAmount, 1.0);\n" +
      "  return output;\n" +
      "}\n";
    ...
    final int uniformBufferSize = Matrix4d.FLOAT_BYTES + Vector3d.FLOAT_BYTES;
    _uniformBuffer =
  ...
  @Nonnull
  private Float32Array getTransformationMatrix( @Nonnull final Vector3d eyePosition )
  {
  ...
  private void renderFrame()
  {
    ...
    eyePosition.mul( new Matrix4d().rotateY( Math.PI * ( System.currentTimeMillis() / 5000.0 ) ) );

    final Float32Array transformationMatrix = getTransformationMatrix( eyePosition );
    _device.queue().writeBuffer( _uniformBuffer,
                                 0,
                                 transformationMatrix.buffer(),
                                 transformationMatrix.byteOffset(),
                                 transformationMatrix.byteLength() );
    final Float32Array eyeArray = new Float32Array( eyePosition.toArray() );
    _device.queue().writeBuffer( _uniformBuffer,
